...
const generateMockData = (projectType, days) => {
  ...
  const count = isA ? days * 12 : days * 6; // 增加数据密度
...
const useDataProcessor = (project, range, filter) => {
  ...
  return useMemo(() => {
    ...
    return {
      ...
      filteredContributors: filteredUsers.sort((a, b) => b.count - a.count).slice(0, 20), // 增加显示数量
      rawData
    };
  }, [rawData, filter]);
};
